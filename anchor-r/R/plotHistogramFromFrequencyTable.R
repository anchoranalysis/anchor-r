...
plotHistogramFromFrequencyTable <- function( freqTable, maxHistBin, minHistBin, name, xAxisLabel, extraFun=NULL ) {
  # Plots a histogram from a frequency table.
  #
  #
  # Args:
  #   freqTable: data.frame or matrix. assumes first column are values, second column are counts.
  ...
  p = (ggplot(data=freqTable,mapping=aes(x=val,y=count))
          + geom_histogram(stat='identity', colour='red', fill='red')  
          + coord_cartesian(xlim = c(minHistBin, maxHistBin))
          + ggtitle(name)
          + scale_y_continuous(labels = scientific)
          + xlab(xAxisLabel)
          + theme(plot.title = element_text(lineheight=.8, face="bold"))
          + geom_density(colour="black", adjust=4, stat='identity'));
  if (!is.null(extraFun)) {
